...
async function getMpListed(amountMomo) {
  ...
  try {
    let mpListed = await axios.get(
      "https://nftapi.mobox.io/auction/search/BNB?page=1&limit=" +
        amountMomo +
    ...
    for (let ii = mpListed.length - 1; ii >= 0; ii--) {
      ...
      priceDec_.push(
        Number((Number(mpListed[ii].nowPrice) / 10 ** 9).toFixed(2))
      );
...
function checkIdPrice(idCheck_, amountCheck_, lvCheck_, lvHashCheck_) {
  ...
  if (
    pricePerHash * lvHashCheck_ > priceCheck &&
    pricePerHash * lvHashCheck_ < budget
  ) {
...
async function divideCanBuy() {
  ...
  for (let index = 0; index < indexCanBuy.length; index++) {
    if (Number(uptimes_[indexCanBuy[index]]) >= uptimesCache) {
      ...
      price_.push(
        Number(nowPrices_[indexCanBuy[index]] + 10 ** 5).toFixed() + "000000000"
      );
...
async function getMinPrice() {
  ...
  for (let index = 0; index < profit.length; index++) {
    price.push(
      Number((priceRaw[index] * 0.95 - profit[index] - 0.06).toFixed(3))
    );
  }
  minCommon = price[0];
  minUncommon = price[1];
...
const profit = [-10.1, 0.1, 0.15, 4, 8, 100]; //profit per mom
const profitName = 30;
