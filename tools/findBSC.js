...
async function getMpListed(amountTx) {
    ...
    try {
        ...
        for (let ii = 0; ii < mpListed.length; ii++) {
            ...
            uptimes_.push(Number(mpListed[ii].timeStamp));
            hashrates_.push(1);
            lvHashrate_.push(1);
            lvMomo_.push(1);
            if (decodedData.ids_.length == 0) {
                //>>>>=Rare
                buyMode.push("Pro");
                amounts_.push(["1"]);
                ids_.push([decodedData.tokenId]);
            } else {
                decodedData.ids_.length == 1 ? buyMode.push("Normal") : buyMode.push("Bundle");
                amounts_.push(decodedData.amounts_);
                ids_.push(decodedData.ids_);
            }
        }
...
let cacheBlock = 0;
const minBudget = 30;
const gasPriceMin = configJson.gasPriceMinBid;
