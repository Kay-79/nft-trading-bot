...
const web3 = new Web3(
  new Web3.providers.HttpProvider("https://bsc-testnet.publicnode.com")
);
...
async function setup(Private_Key_) {
  ...
  if (Bid == true) {
    ...
    if (
      index_[0] != "" &&
      Date.now() / 1000 > Number(startTime_[0]) + timeWait
    ) {
      ...
      for (let index = 0; index < gasPriceScanRaw.length; index++) {
        gasPriceScan[index] = Number(
          (Number(gasPriceScanRaw[index]) * 10 ** 9).toFixed()
        );
      }
      ...
      if (
        false ||
        Date.now() / 1000 < Number(startTime_[0]) + timeWait + overTime
      ) {
        var tx = [];
        ...
        try {
          ...
          for (let index = 0; index < tx.length; index++) {
            ...
            if (tx.length == 1) {
              ...
              console.log(
                "Successful bid! At block:",
                biding[index].blockNumber
              );
            } else {
