...
async function checkChangePrice(indexId) {
  ...
  if (myAccounts.includes(data3[0].auctor)) {
    ...
    priceSell[indexId] = (
      Number(data3[0].nowPrice) / 10 ** 9 -
      minChange
    ).toFixed(3);
    for (let index_q = 0; index_q < data3.length; index_q++) {
      if (
        Number(Date.now() / 1000).toFixed() - Number(data3[index_q].uptime) >
        timeWait
      ) {
        if (myAccounts.includes(data3[index_q].auctor)) {
          priceSell[indexId] = (
            Number(data3[index_q].nowPrice) /
            10 ** 9
          ).toFixed(3);
        } else {
          priceSell[indexId] = (
...
async function getPriceToSell(address, boolMin) {
  ...
  for (let ii = 0; ii < priceList.length; ii++) {
    for (let jj = 0; jj < priceList[ii].length; jj++) {
      priceList[ii][jj] =
        Math.round(Number(priceList[ii][jj]) * 10 ** 5).toString() +
        "0000000000000";
